...
namespace Condor.Core
{
    ...
    {
        public static void Send<TModel>(string templateName, string toEmailAddress, string name, string subject, TModel model)
        {
            var toEmailAddresses = new string[1] { toEmailAddress };
            var names = new string[1] { name };
            Send<TModel>(templateName, toEmailAddresses, names, subject, model);
        }
